...
final class KeyValueStore: @unchecked Sendable {
	...
	func incr(_ key: String) -> Result<Int, StoreError> {
		queue.sync {
			if isExpired(key) {
				remove(key)
			}
	...
	func expire(_ key: String, seconds: Int) -> Int {
		queue.sync {
			if isExpired(key) {
				remove(key)
				return 0
...
struct RespParser {
	...
	static func parseCommand(from buffer: inout Data) -> ParseResult {
...
struct RespEncoder {
	static func simple(_ message: String) -> Data {
	...
	static func bulk(_ value: String?) -> Data {
...
final class Client: @unchecked Sendable {
	...
	init(connection: NWConnection, store: KeyValueStore) {
